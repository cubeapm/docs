...
# Sign in with PingIdentity

To enable Sign in with PingIdentity, you need to create an Application in your PingIdentity account. The following steps guide you through the process:

1. Go to the PingIdentity Admin Console and select the appropriate environment.
...
1. Navigate to Applications > Add Application. Fill **CubeAPM** in Application Name, select **OIDC Web App** in Application Type, and click Save.
   ...
   ![Create Application](/img/configure/oauth/pingidentity/create-app.png)
...
1. On the Application Overview page that appears next, **click the switch next to the application name to enable the application**.
   ...
   ![Application Credentials](/img/configure/oauth/pingidentity/app-credentials.png)

1. Go to **Configuration** tab and click on **Edit** button.

   ![Application Configuration](/img/configure/oauth/pingidentity/app-configuration.png)
   ...
   ![Redirect URI](/img/configure/oauth/pingidentity/app-redirect-uri.png)
...
1. Go to **Resources** tab, click **Edit**, and add **email**, **openid** and **profile** scopes. You may need to scroll down the list to find the scopes.
   ...
   ![Redirect URI](/img/configure/oauth/pingidentity/oidc-scopes.png)
