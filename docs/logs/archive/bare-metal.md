...
1.  Grant read/write permissions to CubeAPM on the bucket. 
    ...
    <Tabs>
      <TabItem value="AWS" label="AWS">
        In **AWS** this can be done in any one of the following ways either by attaching appropriate role to the server instance on which CubeAPM is running, or by creating access keys and providing the same to JuiceFS.
        ...
        In **GCP** this can be done in any one of the following ways either by attaching appropriate role to the server instance on which CubeAPM is running, or by creating service account and providing the access to it and use that service account.

        1. Attach the `storage.objectAdmin` to the GCP VM default service account.
        ...
        1. Create a service account and grant the `storage.objectAdmin` permission to it after granting the permission stop the cubeapm instance and **edit** the instance configuration.
            ...
            1. After settingup Identity and API access, under **Access scopes** select any of the scope either *default* or *full access to all Cloud APIs* and save it.
