...
id: fluentbit-k8s
title: "Fluent Bit"
slug: /logs/ingestion/Kubernetes/fluentbit-k8s
---
...
### Using Fluent Bit Inside Kubernetes Cluster.

Fluent Bit inside Kubernetes Cluster refers to deploying and running the Fluent Bit inside a Kubernetes cluster.

:::info
- To capture logs from the running applications inside kubernetes cluster, You have to deploy the Fluent Bit as a ***daemonset***.
:::

### Installation

To deploy the Fluent Bit inside a Kubernetes cluster, follow the steps below:

1. **Add the Fluent Bit Helm chart repository.**
...
2. **Copy the files below and save as** `fluentbit-daemonset.yaml`. **Edit them to customize the configuration as per your requirements.**

    <details>
    <summary>fluentbit-daemonset.yaml</summary>
    ```yaml
    ...
    config:
        ...
        inputs: |
            [INPUT]
                Name tail
                Path /var/log/pods/*java-springboot*/*/*.log
                Exclude_Path /var/log/pods/*fluent-bit*/*/*.log
                Tag default_java-springboot-*
                Mem_Buf_Limit 5MB
                Skip_Long_Lines On
                Read_From_Head Off
                Refresh_Interval 5
        ...
        filters: |
            ...
            [FILTER]
                Name lua
                Match *
                Script /fluent-bit/etc/conf/mask_password.lua
                Call mask_password
        ...
        customParsers: |
            ...
            [MULTILINE_PARSER]
                ...
                Type regex
                Flush_Timeout 1000
                Rule "start_state" "/^(\d{4}-\d{2}-\d{2}|Exception|Error|Throwable)/" "cont"
                Rule "cont" "/^(?!(\d{4}-\d{2}-\d{2}|Exception|Error|Throwable))/" "cont"

            [PARSER]
                Name named-capture-test
                Format regex
                Regex /^(?<timestamp>\d+-\d+-\d+ \d+:\d+:\d+\.\d+)\s+\[[^\]]+\]\s+(?<severity>[A-Z]+)\s+\S+\s+-\s+(?<log>.*)$/m
                Time_Key timestamp
    ...
    volumeMounts:
        ...
        # - name: luaScripts
        #   mountPath: /fluent-bit/etc/conf/mask_password.lua
...
3. **Install the fluentbit using the following commands:**

   ```bash
   helm install fluent-bit fluent/fluent-bit -f fluentbit-daemonset.yaml
   ```

   ```bash
   # Use the following commands to update if already installed.
   helm upgrade fluent-bit fluent/fluent-bit -f fluentbit-daemonset.yaml
   ```

4. **Check the status of the fluentbit daemonset:**

    ```bash
    kubectl get daemonset fluent-bit -n <namespace>
    ```

5. **Check the status of the fluentbit pods:**

    ```bash
    kubectl get pods -n <namespace>
    ```

6. **Check the logs of the fluentbit pods:**

    ```bash
    kubectl logs -n <namespace> -l app=<fluentbit-daemonset-name>
    ```

7. **Check the Logs in CubeAPM:**
